...
function processStoreString(s) {
	...
	var areas_columns_count = Math.round(areas_count / 8) + 1;
	...
	for (var area_no = 0; area_no < areas_count; ++area_no) {
		...
		for (var row = 0; row < rows_count; ++row) {
			...
			for (var column = 0; column < columns_count; ++column) {
				...
				column_e.appendChild(document.createElement("br"));
				...
				if (row > 0) {
					var t1 = column_e.className;
					var t2 = area_view.lastChild.childNodes.item(column).className;
					if (t1 != t2) {
